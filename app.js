...
document.addEventListener('DOMContentLoaded', async () => {
    // Check server securely for active session instead of blindly trusting localStorage
    const authCheck = await fetch(`${API_URL}?action=check_auth`).then(r => r.json());
    ...
    if (authCheck.success) {
        ...
        if (savedSharedCode) {
            loadSharedUser(savedSharedCode);
        } else {
            fetchReadings().then(() => navigate('dashboard'));
        }
    }
...
async function apiCall(action, payload = {}) {
    ...
    } catch (e) { showModal('Error', 'Connection failed.'); return { success: false }; }
}

async function fetchReadings() {
    viewingSharedProfile = null;
    const res = await fetch(`${API_URL}?action=get_readings`);
...
async function login() {
    ...
    } else { showModal('Login Failed', data.message); }
}
...
async function loadSharedUser(code) {
    ...
    if (res.success) {
        ...
        showModal('Error', 'Invalid share code.');
        localStorage.removeItem('vitalTrackSharedCode');
    }
}
...
function renderDashboard(container) {
    ...
    const recent = currentReadings.filter(r => new Date(r.date) >= sevenDaysAgo);
    ...
    const bpStat = getBPStatus(avgSys, avgDia);
    const pulseStat = getPulseStatus(avgPulse);
    const oxyStat = getOxygenStatus(avgOxy);

    const noDataText = `<p style="font-size: 1rem; margin-top: 5px; color: var(--text-muted);">No data in the past seven days</p>`;
    const bpHtml = bpCount ? `<p style="font-size: 1.2rem; margin-top: 5px;">${escapeHTML(avgSys)}/${escapeHTML(avgDia)} <span class="${bpStat.class}">(${bpStat.text})</span></p>` : noDataText;
    const pulseHtml = pCount ? `<p style="font-size: 1.2rem; margin-top: 5px;">${escapeHTML(avgPulse)} bpm <span class="${pulseStat.class}">(${pulseStat.text})</span></p>` : noDataText;
    const oxyHtml = oCount ? `<p style="font-size: 1.2rem; margin-top: 5px;">${escapeHTML(avgOxy)}% <span class="${oxyStat.class}">(${oxyStat.text})</span></p>` : noDataText;

    const actionDisabled = viewingSharedProfile ? 'disabled style="opacity:0.5"' : '';

    let recentTableHtml = `<div style="overflow-x: auto;"><table class="admin-table" style="font-size:0.9em; margin-bottom: 0;">
        <thead><tr><th>Date</th><th>BP</th><th>Pulse</th><th>Oxygen</th></tr></thead>
        <tbody>`;

    if (recent.length === 0) {
        recentTableHtml += `<tr><td colspan="4" style="text-align:center; color:var(--text-muted); padding: 20px;">No data in the past seven days</td></tr>`;
    } else {
        recent.forEach(r => {
            const bp = getBPStatus(r.sys, r.dia); const pu = getPulseStatus(r.pulse); const ox = getOxygenStatus(r.oxygen);
            recentTableHtml += `<tr>
                <td>${formatDate(r.date)}<br><small>${escapeHTML(r.time)}</small></td>
                <td>${r.sys ? `${escapeHTML(r.sys)}/${escapeHTML(r.dia)} <br><span class="${bp.class}">(${bp.text})</span>` : '-'}</td>
                <td>${r.pulse ? `${escapeHTML(r.pulse)} <br><span class="${pu.class}">(${pu.text})</span>` : '-'}</td>
                <td>${r.oxygen ? `${escapeHTML(r.oxygen)}% <br><span class="${ox.class}">(${ox.text})</span>` : '-'}</td>
            </tr>`;
        });
    }
    ...
    container.innerHTML += `
        <div class="dashboard-grid">
            <div class="card card-full">
                ...
                <div class="dashboard-averages-wrapper">
                    <div class="card ${bpStat.border}" style="flex: 1; min-width: 200px; padding: 20px;">
                        <h4>Blood Pressure</h4>
                        ${bpHtml}
                    </div>
                    <div class="card ${pulseStat.border}" style="flex: 1; min-width: 200px; padding: 20px;">
                        <h4>Pulse Rate</h4>
                        ${pulseHtml}
                    </div>
                    <div class="card ${oxyStat.border}" style="flex: 1; min-width: 200px; padding: 20px;">
                        <h4>Oxygen Level</h4>
                        ${oxyHtml}
                    </div>
                </div>
            ...
            <div class="card">
                <h3>Quick Actions</h3>
                <button class="btn-primary" ${actionDisabled} onclick="navigate('entry', {period: 'Morning'})">Record Morning</button>
                <button class="btn-primary" ${actionDisabled} onclick="navigate('entry', {period: 'Evening'})">Record Evening</button>
                <button class="btn-primary" ${actionDisabled} style="background: var(--text-muted); margin-bottom: 0;" onclick="navigate('entry', {period: 'Unknown'})">Record Daily</button>
            </div>
...
function renderEntry(container, editId = null, defaultPeriod = 'Unknown') {
    ...
    if (editId) {
        const found = currentReadings.find(x => x.id == editId);
        if (found) r = found;
    }
    ...
    window.toggleInputs = function () {
        ...
        document.getElementById('bpInputs').classList.toggle('hidden', type === 'oxy_pulse');
        document.getElementById('oxyInput').classList.toggle('hidden', type === 'bp_pulse');
    };
}
...
async function saveEntry() {
    const payload = {
        ...
        date: document.getElementById('entryDate').value, time: document.getElementById('entryTime').value,
        period: document.getElementById('entryPeriod').value,
        sys: document.getElementById('sys')?.value || null, dia: document.getElementById('dia')?.value || null,
        pulse: document.getElementById('pulse')?.value || null, oxygen: document.getElementById('oxy')?.value || null,
        notes: document.getElementById('notes').value
    };

    const action = editingReadingId ? 'update_reading' : 'save_reading';
    const res = await apiCall(action, payload);
    if (res.success) { editingReadingId = null; await fetchReadings(); navigate('dashboard'); }
}

async function deleteEntry(id) {
    if (confirm("Are you sure you want to delete this reading?")) {
        const res = await apiCall('delete_reading', { reading_id: id });
        if (res.success) { await fetchReadings(); navigate('history'); }
    }
}
    ...
    container.innerHTML += `
        <div class="card">
            ...
            <div style="display:flex; gap:10px; margin-bottom: 20px; flex-wrap: wrap;">
                <div style="flex:1"><label>Start Date</label><input type="date" id="histStart" value="${threeMonthsAgo.toISOString().split('T')[0]}" onchange="updateHistoryView()"></div>
                <div style="flex:1"><label>End Date</label><input type="date" id="histEnd" value="${today.toISOString().split('T')[0]}" onchange="updateHistoryView()"></div>
            </div>
            <canvas id="historyChart" style="width:100%; max-height: 350px;"></canvas>
        </div>
        <div class="card">
            <h3>Historical Results</h3>
            <div style="overflow-x: auto;">
                <table class="admin-table">
                    <thead><tr><th>Date</th><th>BP</th><th>Pulse</th><th>Oxygen</th>${!viewingSharedProfile ? '<th>Actions</th>' : ''}</tr></thead>
...
function updateHistoryView() {
    ...
    const filtered = currentReadings.filter(r => {
        return (!startStr || r.date >= startStr) && (!endStr || r.date <= endStr);
    }).reverse();
    ...
    [...filtered].reverse().forEach(r => {
        const bp = getBPStatus(r.sys, r.dia); const pu = getPulseStatus(r.pulse); const ox = getOxygenStatus(r.oxygen);
        const actionHtml = !viewingSharedProfile ? `<td>
            <button class="btn-primary" onclick="navigate('entry', {editId: ${escapeHTML(r.id)}})" style="padding: 6px 12px; font-size: 0.9rem; width: auto; margin-right: 5px; margin-bottom: 5px; display: inline-block;">Edit</button>
            <button class="btn-primary" onclick="deleteEntry(${escapeHTML(r.id)})" style="padding: 6px 12px; font-size: 0.9rem; width: auto; margin-bottom: 5px; display: inline-block; background-color: var(--status-very-high); box-shadow: none;">Delete</button>
        </td>` : '';
        tbody.innerHTML += `<tr>
            <td>${formatDate(r.date)}<br><small>${escapeHTML(r.time)}</small></td>
            <td>${r.sys ? `${escapeHTML(r.sys)}/${escapeHTML(r.dia)} <br><span class="${bp.class}">(${bp.text})</span>` : '-'}</td>
    ...
    historyChartInstance = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [
                { label: 'Systolic', data: filtered.map(r => r.sys), borderColor: '#ef4444', tension: 0.1, spanGaps: true },
                { label: 'Diastolic', data: filtered.map(r => r.dia), borderColor: '#f87171', tension: 0.1, spanGaps: true },
                { label: 'Pulse', data: filtered.map(r => r.pulse), borderColor: '#16a34a', tension: 0.1, spanGaps: true },
                { label: 'Oxygen', data: filtered.map(r => r.oxygen), borderColor: '#3b82f6', tension: 0.1, spanGaps: true }
            ]
        },
...
function generatePDF() {
    const startStr = document.getElementById('repStart').value;
    const endStr = document.getElementById('repEnd').value;
    const incBP = document.getElementById('incBP').checked;
    const incPulse = document.getElementById('incPulse').checked;
    const incOxy = document.getElementById('incOxy').checked;
    ...
    const patientName = escapeHTML(targetProfile.name) || 'Unknown Patient';
    let patientDob = escapeHTML(targetProfile.dob) || 'DD/MM/YYYY';

    if (pdfChartInstance) pdfChartInstance.destroy();

    const ctx = document.getElementById('pdfHiddenChart').getContext('2d');
    ctx.clearRect(0, 0, 800, 400);

    const chartDatasets = [];

    if (incBP) {
        chartDatasets.push({ label: 'Systolic', data: filtered.map(r => r.sys || null), borderColor: '#ef4444', backgroundColor: 'transparent', spanGaps: true });
        chartDatasets.push({ label: 'Diastolic', data: filtered.map(r => r.dia || null), borderColor: '#f87171', backgroundColor: 'transparent', spanGaps: true });
    }
    ...
    pdfChartInstance = new Chart(ctx, {
        type: 'line',
        data: { labels: filtered.map(r => formatDate(r.date)), datasets: chartDatasets },
        options: { responsive: false, animation: false }
    });

    setTimeout(() => {
        const chartImg = document.getElementById('pdfHiddenChart').toDataURL("image/png", 1.0);
        const { jsPDF } = window.jspdf;
        const doc = new jsPDF();

        // 1. Header (Updated to say "Date of Birth")
        doc.setFontSize(16); doc.text("Patient Report", 14, 20);
        doc.setFontSize(10); doc.text(`Name: ${patientName}`, 14, 30); doc.text(`Date of Birth: ${patientDob}`, 14, 35);
        ...
        doc.autoTable({
            startY: 135, head: tableHead, body: tableBody,
            styles: { fontSize: 9, halign: 'center' }, headStyles: { fillColor: [29, 78, 216] },
            didParseCell: function (data) {
                if (data.section === 'body' && data.column.index > 1) {
                    data.cell.customText = data.cell.raw;
                    const lines = String(data.cell.customText).split('\n');
                    data.cell.text = lines.map(() => '');
                }
            },
            didDrawCell: function (data) {
                if (data.section === 'body' && data.column.index > 1 && data.cell.customText) {
                    ...
                    if (raw !== '-') {
                        ...
                        doc.text(val, centerX, centerY - 2.5, { align: 'center', baseline: 'middle' });

                        if (stat.includes('(Very High)') || stat.includes('(Very Low)')) {
                            doc.setTextColor(153, 27, 27); doc.setFont("helvetica", "bold");
                        } else if (stat.includes('(High)') || stat.includes('(Low)')) {
                            doc.setTextColor(220, 38, 38); doc.setFont("helvetica", "bold");
                        } else if (stat.includes('(Good)')) {
                        ...
                        doc.setFontSize(9); doc.setFont("helvetica", "normal"); doc.setTextColor(40, 40, 40);
                        doc.text('-', centerX, centerY, { align: 'center', baseline: 'middle' });
                    }
                }
            }
        });

        // 4. Footers (Updated to say "Date of Birth")
        const pageCount = doc.internal.getNumberOfPages();
        for (let i = 1; i <= pageCount; i++) {
            doc.setPage(i); doc.setFontSize(8); doc.setTextColor(100); doc.setFont("helvetica", "normal");
            doc.text(`Page ${i}`, 14, doc.internal.pageSize.height - 10);
            doc.text(`Patient Name: ${patientName} | Date of Birth: ${patientDob}`, doc.internal.pageSize.width - 14, doc.internal.pageSize.height - 10, { align: 'right' });
        }
...
function generateCSV() {
    ...
    link.download = `VitalTrack_Data_${new Date().toISOString().split('T')[0]}.csv`;
...
async function renderSettings(container) {
            ...
            <div class="card">
                ...
                <div class="admin-grid" style="margin-bottom: 30px;">
                    <div class="admin-form">
                        <h4 style="margin-bottom:15px;">Add New User</h4>
                        ...
                        <button class="btn-primary" style="margin-bottom:0;" onclick="adminAddUser()">Add User</button>
                    </div>
                    <div class="admin-form">
                        <h4 style="margin-bottom:15px;">Reset User Password</h4>
                        <select id="resetUserId" style="margin-bottom:10px;"><option value="">Select User...</option></select>
                        <input type="password" id="resetPassword" placeholder="New Password" style="margin-bottom:10px;">
                        <button class="btn-primary" style="background: var(--text-muted); margin-bottom:0;" onclick="adminResetPassword()">Reset Password</button>
                    </div>
                </div>
                <h4 style="margin-bottom: 10px;">Manage Existing Users</h4>
                <div style="overflow-x:auto;">
                    <table class="admin-table">
                        <thead><tr><th>User</th><th>Role</th><th>Action</th></tr></thead>
                        <tbody id="adminUserList"><tr><td colspan="3">Loading...</td></tr></tbody>
                    </table>
                </div>
            </div>`;
    }
    ...
    container.innerHTML += `
        ...
        <div class="card">
            <h3>Profile</h3>
            <label>Full Name</label> <input type="text" id="profName" value="${escapeHTML(currentUser?.name || '')}">
            <label>Date of Birth (DD/MM/YYYY)</label> <input type="text" id="profDOB" placeholder="DD/MM/YYYY" value="${escapeHTML(currentUser?.dob || '')}">
...
async function saveAndLoadSharedCode() {
    const code = document.getElementById('viewShareCode').value.trim().toUpperCase();
    if (code) loadSharedUser(code);
}

function copyShareCode() {
    const code = document.getElementById('myShareCode').value;
    if (!code || code === 'No code generated') return showModal('Error', 'No code generated yet.');
    navigator.clipboard.writeText(code).then(() => {
        showModal('Success', 'Share code copied to clipboard!');
    }).catch(() => {
        showModal('Error', 'Failed to copy code. Please manually select and copy it.');
    });
}

async function saveProfile() {
    const name = document.getElementById('profName').value; const dob = document.getElementById('profDOB').value;
    const res = await apiCall('update_profile', { name, dob });
    if (res.success) { currentUser.name = name; currentUser.dob = dob; showModal('Success', 'Profile updated.'); }
}

async function genShareCode() {
    if (currentUser?.share_code && !confirm("Generating a new code will permanently revoke access for anyone using the old code. Do you want to continue?")) return;
...
async function downloadJSON() {
    const res = await fetch(`${API_URL}?action=backup_data`);
    const data = await res.json();
    if (data.success) {
        const blob = new Blob([JSON.stringify(data.backup, null, 2)], { type: 'application/json' });
        const link = document.createElement("a"); link.href = URL.createObjectURL(blob); link.download = "VitalTrack_Backup.json"; link.click();
    }
}
...
function uploadJSON(event) {
    ...
    reader.onload = async (e) => {
        try {
            const readings = JSON.parse(e.target.result); const res = await apiCall('restore_data', { readings });
            if (res.success) { showModal('Success', 'Data restored successfully.'); fetchReadings(); }
        } catch (err) { showModal('Error', 'Invalid JSON file.'); }
    };
...
async function loadAdminUsers() {
    const res = await apiCall('admin_action', { task: 'get_users' });
    if (res.success) {
        const tbody = document.getElementById('adminUserList'); const resetDropdown = document.getElementById('resetUserId');
        tbody.innerHTML = ''; resetDropdown.innerHTML = '<option value="">Select User...</option>';
        res.users.forEach(u => {
            tbody.innerHTML += `<tr><td><strong>${escapeHTML(u.username)}</strong><br><small style="color:var(--text-muted);">${escapeHTML(u.name || 'No Name')}</small></td>
                <td style="text-transform: capitalize;">${escapeHTML(u.role)}</td>
                <td><button onclick="deleteAdminUser(${escapeHTML(u.id)})" style="padding:8px 12px; background:var(--status-very-high); color:#fff; border:none; border-radius:6px; cursor:pointer;">Delete</button></td></tr>`;
            resetDropdown.innerHTML += `<option value="${escapeHTML(u.id)}">${escapeHTML(u.username)}</option>`;
        });
    }
}
async function adminAddUser() {
    const user = document.getElementById('newUsername').value; const pass = document.getElementById('newPassword').value; const role = document.getElementById('newRole').value;
    if (!user || !pass) return showModal('Error', 'Username and password required.');
    const res = await apiCall('admin_action', { task: 'add_user', new_username: user, new_password: pass, new_role: role });
    if (res.success) { showModal('Success', 'User added.'); document.getElementById('newUsername').value = ''; document.getElementById('newPassword').value = ''; loadAdminUsers(); }
    else { showModal('Error', 'Could not add user. Username may already exist.'); }
}
async function adminResetPassword() {
    const userId = document.getElementById('resetUserId').value; const newPass = document.getElementById('resetPassword').value;
    if (!userId || !newPass) return showModal('Error', 'Select a user and enter a new password.');
    const res = await apiCall('admin_action', { task: 'reset_password', target_user_id: userId, new_password: newPass });
    if (res.success) { showModal('Success', 'Password reset.'); document.getElementById('resetPassword').value = ''; }
}
